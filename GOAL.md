...
- The default page on port 4182 is now a configurable composer with browser-wallet Sepolia setup/signing and local test-account execution. Tokens, limits, fees, reserves, module source/parameters and pool settings are editable. Nondefault and edited-module routes settled successfully locally; public inspection was checked without broadcasting.
- The `/transaction` page is a read-only public Sepolia transaction viewer: one transaction frame, six stages, exact transfers, all logs/input/gas, live RPC confirmation/finality refresh and explicit saved-evidence fallback. The local interactive strategy lab remains at `/atomic`. Keystore secrets are not retained.
