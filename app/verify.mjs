import { mkdtemp, readFile, writeFile, mkdir, symlink, rm, cp } from 'node:fs/promises';
...
export function validateSource(source) {
 ...
 // Tokenize comments and strings together: a comment marker inside a string is data.
...
export async function verifyCandidate(source,{fuzzRuns=128,onEvent=()=>{}}={}) {
 if(!Number.isInteger(fuzzRuns)||fuzzRuns<32||fuzzRuns>1024)throw new Error('fuzzRuns must be 32..1024');
 ...
 try {
  ...
  const upstream=(await readFile(join(root,'test/Upstream.t.sol'),'utf8')).replace('../app/fixtures/Candidate.good.sol','../src/Candidate.sol');
  const bridge=(await readFile(join(root,'test/Studio.t.sol'),'utf8')).replace('../app/fixtures/Candidate.good.sol','../src/Candidate.sol');
  await writeFile(join(dir,'test/Upstream.t.sol'),upstream);await writeFile(join(dir,'test/Studio.t.sol'),bridge);
  await cp(join(root,'contracts'),join(dir,'contracts'),{recursive:true});
  await symlink(join(root,'vendor'),join(dir,'vendor'),'dir');
  await writeFile(join(dir,'test/Candidate.t.sol'),harness);
  await symlink(join(root,'node_modules'),join(dir,'node_modules'),'dir');
  await writeFile(join(dir,'foundry.toml'),'[profile.default]\nsolc_version="0.8.30"\nevm_version="cancun"\noptimizer=true\noptimizer_runs=700\nvia_ir=true\nffi=false\nfs_permissions=[]\nremappings=["forge-std/=node_modules/forge-std/src/","swap-vm/=vendor/swap-vm/","@openzeppelin/contracts/=node_modules/@openzeppelin/contracts/","@1inch/solidity-utils/=node_modules/@1inch/solidity-utils/","@1inch/aqua/=node_modules/@1inch/aqua/"]\n');
  onEvent({type:'verification',message:'Compiling isolated candidate and running fixed Foundry campaign'});
  let output,exitCode=0;
  try {({stdout:output}=await run(forge,['test','--root',dir,'--json','--match-contract','^(CandidateCampaign|UpstreamTest)$','--fuzz-runs',String(fuzzRuns),'--fuzz-seed','0x53545544494f'],{cwd:dir,timeout:120000,maxBuffer:4_000_000,env:{...Object.fromEntries(Object.entries(process.env).filter(([key])=>!key.startsWith('FOUNDRY_')&&!key.startsWith('DAPP_'))),FOUNDRY_FFI:'false'}}));}
  ...
  const passed=exitCode===0 && checks.length>=11 && checks.every(c=>c.passed);
  const report={passed,checks,counterexamples:checks.filter(c=>!c.passed).map(c=>({name:c.name,reason:c.output,inputs:c.counterexample})),sourceHash:digest(source),harnessHash:digest(harness+upstream+bridge),compiler:'solc 0.8.30, optimizer 700, viaIR, Cancun',fuzzRuns,seed:'0x53545544494f',scope:'Pricing properties on reserves 1e24/1e24 and amounts 1e12..1e21; balance checks extend to 1e27. This campaign is testing, not a proof.',skipped:[],upstream:{version:'ac06e1bac021cd1983dc7c44d1f69b4b8861a945',allSkipFlags:false,externalAuthorFeeBps:0,scope:'CoreInvariants against this candidate through actual Aqua/Studio settlement; author-fee integration is tested separately.'},durationMs:Date.now()-started};
  if(passed){
   ...
   report.initCodeHash=keccak256(report.bytecode);report.runtimeCodeHash=keccak256(report.runtimeBytecode);
